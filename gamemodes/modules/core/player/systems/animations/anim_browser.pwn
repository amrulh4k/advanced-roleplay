...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == D_ANIM_LIBRARIES && response) {
		...
		if(0 < idx < MAX_ANIMS) {
			...
			ShowPlayerDialog(playerid, D_ANIM_IDX, DIALOG_STYLE_INPUT, "ID do Anim", "Digite o ID da animação (idx)\nentre 0 e "#MAX_ANIMS":", "Inserir", "Cancelar");
		}
	}
	return 1;
}
...
FormatAnimSettingsMenu(playerid) {
	...
	ShowPlayerDialog(playerid, D_ANIM_SETTINGS, DIALOG_STYLE_LIST, "Configurações da Animação", list,"Mudar", "<<<");
}
...
LoadPlayerTextDraws(playerid) {
	...
	guiAnimLib                      =CreatePlayerTextDraw(playerid, 320.0, 375.0, "BUSCAR ANIM");
	...
	guiAnimName                     =CreatePlayerTextDraw(playerid, 320.0, 390.0, "NOME DO ANIM");
	...
	guiExitBrowser                  =CreatePlayerTextDraw(playerid, 320.000000, 410.000000, "Sair");
